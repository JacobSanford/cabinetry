...
class ShelfFFShelf {
   ...
   *
   * @return mixed
   *   An array of information about the fitting of the part, FALSE otherwise.
   */
  public function partFits(CabinetryPartInterface $part) {
    if ($part->getHeight() > $this->height) {
      return FALSE;
    }

    if ($part->getWidth() > $this->remain && $this->canRotate == FALSE) {
      return FALSE;
    }

    // If applicable, check if a rotation of the part makes it fit.
    if ($this->canRotate == TRUE && $part->getHeight() <= $this->remain && $part->getWidth() <= $this->height) {
      $rotated_balance = $this->height - $part->getWidth();
    }

    // Check if non-rotated fits better than rotated.
    if ($part->getWidth() <= $this->remain) {
      if (!isset($rotated_balance) || $this->height - $part->getHeight() <= $rotated_balance) {
        return [
          'balance' => $this->remain - $part->getWidth(),
          'rotated' => FALSE,
        ];
      }
    }

    // If the rotated part fit, return it.
